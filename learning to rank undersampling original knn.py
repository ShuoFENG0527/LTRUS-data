    ...
    for train, test in skf.split(de_x, de_y):
        ...
        for sub_train, sub_test in skf.split(de_train_x, de_train_y):
            ...
            if len(de_sub_train_y[de_sub_train_y == 1]) - 1 >= best_k:
                ...
                de_clf = neighbors.KNeighborsClassifier(n_neighbors=5)
...
for iteration in range(10):
    single = open('D:\\ltrus to reliability\\original\\'+classifier+'radius based oversampling '+str(iteration)+'.csv', 'w',
                  newline='')
    ...
    for inputfile in os.listdir("D:\\datasets\\all\\only first version"):
        ...
        classifier_for_selection = {"knn": neighbors.KNeighborsClassifier(n_neighbors=best_k), "svm": svm.SVC(),
