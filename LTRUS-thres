...
def fit(bound, *param):
    ...
    for train, test in skf.split(de_x, de_y):
        ...
        for sub_train, sub_test in skf.split(de_train_x, de_train_y):
            ...
            de_sub_clean_x = de_sub_clean_x[de_sub_clean_x[:, -1] > range]
            if len(de_sub_clean_x) == 0:
                 return 0
            de_sub_clean_x = de_sub_clean_x[:, 0:-1]
            ...
            # if len(de_sub_train_y[de_sub_train_y == 1]) - 1 >= best_k:
...
for iteration in range(10):
    single = open('D:\\ltrus to reliability\\'+classifier+'ltrus '+str(iteration)+'.csv', 'w',
                  newline='')
    single_writer = csv.writer(single)
    single_writer.writerow(["inputfile", "mcc", "auc", "balance", "fmeasure", "precision", "pd", "pf", "final_ratio"])
    for inputfile in os.listdir("D:\\datasets\\all\\only first version"):
        ...
        optimal_range = optimal_result[-1]
        y = dataset[:, -1]
        x = dataset
        ...
        for train, test in skf.split(x, y):
            ...
            clean_x = clean_x[clean_x[:, -1] > optimal_range]
            clean_x = clean_x[:, 0:-1]
            clean_x[:, 0:-1] = clean_x[:, 0:-1] / optimal_weight
            train_x = np.r_[clean_x, defect_x]
            train_y = train_x[:, -1]
            final_ratio = len(train_y[train_y == 1])/len(train_y)
            train_x = train_x[:, 0:-1]
            clf = classifier_for_selection[classifier]
            # if len(train_y[train_y == 1]) - 1 >= best_k:
            #     clf = classifier_for_selection[classifier]
            # else:
        ...
        single_writer.writerow(
            [inputfile, average_mcc, average_auc, average_balance, average_fmeasure, average_precision, average_recall, average_pf, final_ratio])
